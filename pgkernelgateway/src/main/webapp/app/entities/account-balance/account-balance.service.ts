...
export class AccountBalanceService {
  ...
  protected convertDateFromClient(accountBalance: IAccountBalance): IAccountBalance {
    const copy: IAccountBalance = Object.assign({}, accountBalance, {
      situationDate:
        accountBalance.situationDate != null && accountBalance.situationDate.isValid() ? accountBalance.situationDate.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.situationDate = res.body.situationDate != null ? moment(res.body.situationDate) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((accountBalance: IAccountBalance) => {
        accountBalance.situationDate = accountBalance.situationDate != null ? moment(accountBalance.situationDate) : null;
      });
    }
    return res;
  }
